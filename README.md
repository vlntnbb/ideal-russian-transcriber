...
Выбор LLM для авторизованного пользователя:
- посмотреть текущий выбор: `/llm` (или `/model`)
- выбрать Gemini: `/llm gemini`
- выбрать локальную модель: `/llm local`

Примечание: если выбран Gemini, но `GEMINI_API_KEY` не задан, бот автоматически использует Local (Ollama).
