...
Авторизация:
1) Напишите боту в личку: `/auth <email@bbooster.io>` (или просто отправьте `email@bbooster.io` сообщением) — домен берётся из `AUTH_DOMAIN`
...
## Обработка транскрипций (/process)

Бот хранит последнюю транскрипцию (Whisper + GigaAM) для каждого пользователя в чате.
Дальше можно попросить LLM обработать её по вашему промпту (Gemini — только для авторизованных/whitelist; иначе — локальная LLM):
